<template>
  <el-dialog v-model="visible" title="修改用户" width="40%" :before-close="handleCancel">
    <el-alert
      v-if="usersStore.updateError"
      title="修改用户失败"
      :description="usersStore.updateError"
      type="error"
...
// 表单校验规则
const rules = editUserRules(model.value)
...
const handleOk = () => {
  form.value.validate((valid) => {
    if (valid) {
      ...
      usersStore.update(model.value).then((res) => {
        if (res) {
